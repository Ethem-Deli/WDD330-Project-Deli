...
export function loadFavorites() {
    ...
    if (!favorites.length) { list.innerHTML = '<li><em>No favorites yet</em></li>'; return; }
    list.innerHTML = favorites.map((f, i) => `<li data-i="${i}">${f.title || f.name} <button data-i="${i}" class="removeFavBtn">Remove</button></li>`).join('');
...
export function loadSharedFavoritesFromQuery() {
    ...
    if (params.has('shared')) {
        try {
            ...
            loadFavorites();
        } catch (err) { console.error('Failed to load shared favorites', err); }
    }
}
